...
import { FAV_URL, imageURL } from '../api/apiURLs'
...
const closeDeleteModal = () => {
  ...
  let getAllFav = async() => {
    setLoading(true)
    try {
       let response = await axiosClient.get(FAV_URL.GET_FAV );
  ...
  let remove_fav = async(id) =>{
    ...
    try {
        let response = await axiosClient.delete(FAV_URL.DELETE_FAV(id))
       ...
       <div  className='container'>
          <div className="row">
             {allFav.length > 0 ?
                  ...
                  <div className="item m-3 dropDownMenu text-center p-3 position-relative"   >
